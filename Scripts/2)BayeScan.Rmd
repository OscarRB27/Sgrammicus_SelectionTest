...
(Sg.outliers.bs.All<-Sg.loci[Sg.All.res$pval.adjust<=0.001,])

#Export outliers database
write.table(Sg.outliers.bs.All,file="Analysis/BayeScan/Results/Sg_outliers/Sg.All.outliers.BS.txt",row.names=F,col.names=T,quote=F)
```
...
```{r}
#All sites
(Sg.All.out<-ggplot(Sg.All.res,aes(x=bayescan_number,y=fst))+ #Axis definition
                    ...
                    #annotate("text",x=0.56,y=0.44,label="190699:30",size=3.5)) #Add candidate SNP name
```
...
#Function evaluation for output
#Run function
Eval_P6<-Likelihood_Function_P6(Sg.P6[,"alpha"]) 
#Check results
head(Eval_P6)
...
#Examine which ones are significant now
(Sg.outliers.P6.bs<-Sg.loci[Sg.P6.res$pval.adjust<=0.001,])

#Export outliers database
write.table(Sg.outliers.P6.bs,file="Analysis/BayeScan/Results/Sg_outliers/Sg.P6.outliers.BS.txt",row.names=F,col.names=T,quote=F)
```

4) Explore significant outliers with a plot

```{r}
(Sg.P6.out<-ggplot(Sg.P6.res,aes(x=bayescan_number,y=fst))+ #Axis definition
                    geom_point(size=2,alpha=0.5, #Geometry definition
                    aes(colour=pval.adjust<=0.001))+ #Points color adjusted to p-val.adjust
                    scale_color_manual(values=c("black","red"))+ #Define point colors
                    theme_bw()+ #Canvas definition
                    ylab("FST")+ #y-lab name
                    xlab("Bayescan number")+ #x-lab name
                    ggtitle("P6 sites")+ #Put title
                    labs(color="Adjusted p-values")+ #Rename legend title
                    theme(#plot.title=element_text(hjust=0.5), #Center title
                    axis.title.y=element_text(size=13), #y-axis title size
                    axis.title.x=element_text(size=13)))#+ #x-axis title size
                    #annotate("text",x=0.69,y=0.09,label="73310:109",size=3)) #Add candidate SNP name
```
...
#Examine which ones are significant now
(Sg.outliers.P5.bs<-Sg.loci[Sg.P5.res$pval.adjust<=0.001,])

#Export outliers database
write.table(Sg.outliers.P5.bs,file="Analysis/BayeScan/Results/Sg_outliers/Sg.P5.outliers.BS.txt",row.names=F,col.names=T,quote=F)
```

4) Explore significant outliers with a plot

```{r}
(Sg.P5.out<-ggplot(Sg.P5.res,aes(x=bayescan_number,y=fst))+ #Axis definition
                   geom_point(size=2,alpha=0.5, #Geometry definition
                   aes(colour=pval.adjust<=0.001))+ #Points color adjusted to p-val.adjust
                   scale_color_manual(values=c("black","red"))+ #Define point colors
                   theme_bw()+ #Canvas definition
