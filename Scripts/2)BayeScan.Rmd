...
This analysis is part of my research project and this script will be used to analyze the outputs obtained from the BayeScan software from the SNP data obtained from four different sites where the lizard *Sceloporus grammicus* was sampled. The `plot_bayescan()` function that comes with the BayeScan software will be used for this propose. (Follows M and OE Gaggiotti, 2008).
...
Parameters Forest sites
`Analysis/BayeScan/Program/BayeScan2.1_macos64bits Data/BayeScan/Sg_For_Bayescan`
`-o Analysis/BayeScan/Results/Sg_For/Sg_For_Bayescan_RE1 -threads 4 -n 5000 -thin 20 -nbp 20 -pilot 1000 -burn 10000`

Parameters Agricultural sites
`Analysis/BayeScan/Program/BayeScan2.1_macos64bits Data/BayeScan/Sg_Agr_Bayescan` 
`-o Analysis/BayeScan/Results/Sg_Agr/Sg_Agr_Bayescan_RE1 -threads 6 -n 5000 -thin 20 -nbp 20 -pilot 1000 -burn 10000`
...
5) Analysis and plotting of the outputs
...
Sg.for.res$nb_outliers #Very large number!
```
...
```{r}
#Forest sites
(Sg.For.out<-ggplot(Sg.For.res,aes(x=fst, y=alpha))+ #Axis definition
...
```{r}
#Agricultural sites
(Sg.Agr.out<-ggplot(Sg.Agr.res,aes(x=fst,y=alpha))+ #Axis definition
                   geom_point(size=2,alpha=0.5, #Geometry definition
                   aes(colour=pval.adjust<=0.001))+ #Points color adjusted to p-val.adjust
                   scale_color_manual(values=c("black","red"))+ #Define point colors
                   theme_bw()+ #Canvas definition
                   ...
                   xlab("Fst")+ #x-lab name
                   labs(color="Adjusted p-values")+ #Rename legend title
                   ggtitle("Agricultural sites")+ #Put title
                   theme(plot.title=element_text(hjust=0.5), #Center title
                   axis.title.y=element_text(size=13), #y-axis title size
                   axis.title.x=element_text(size=13))+ #x-axis title size
